...
export const PaymentStep = ({ teamInfo, members, onComplete, onBack }: PaymentStepProps) => {
  ...
  const registrationFee = 3000.00; //  registration fee
  ...
  const handleUploadSubmit = async () => {
    ...
    try {
      const response = await fetch("https://medusa-backend-new-1081502718638.us-central1.run.app/api/payment/upload", {
        method: "POST",
        body: formData
      });
